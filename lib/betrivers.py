...
def _parse_matchup(matchup, names, odds, dates):
    ...
    if "WIN" in matchup.text:
        ...
        if len(ml_odds_elements) == 2: #only if there are 2 elements in the money line odds
            ...
            #get the teams
            teams = matchup.find_elements(By.XPATH, './/span[@class="sc-eYqAFs hZZjng"]')
            for team in teams:
                team_text = team.text
...
def _parse(queue):
    ...
    old_data = driver.find_elements(By.XPATH, '//div[@class="sc-bqoUIo Ldrte"]')
    while True:
        try:
            new_data = driver.find_elements(By.XPATH, '//div[@class="sc-bqoUIo Ldrte"]')
